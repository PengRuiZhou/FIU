...
## Review Round 11（全新角度：性能/Rust GIL/部署升级）
...
| ID | 来源 | 问题 | 决议 |
| -- | ---- | ---- | ---- |
| C-R11-1 | A3 | 首次部署须引擎停止状态——setup.sh 无 stop→upgrade→start guard；live rsync 与 __init__ recovery 竞争 | Accepted — §3.7 INV-CM-DEPLOY-STOPPED |

#### Major
| ID | 来源 | 问题 | 决议 |
| -- | ---- | ---- | ---- |
| M-R11-1 | A1 | sidecar 末行读取须 tail-read 模式（非 readlines） | Accepted — §3.7 note |
| M-R11-2 | A1 | writer retry 是 queue-serial bounded（非 hot-loop），开销不放大 | Accepted — §3.7 note |
| Maj-R11-1 | A2 | spec 未声明 tickfile-writer GIL baseline（tickfile 不在 Phase 21 去 GIL 范围） | Accepted — §3.7 GIL baseline |
| Maj-R11-2 | A2 | Rust tickfile_generate 启用顺序与 sidecar 合并兼容（Python per-row vs Rust whole-string 两路径） | Accepted — §3.7 兼容声明 |
| Maj-R11-3 | A2 | sidecar 第 2 次 fsync 每分钟多持 GIL ~5-20ms，须量化确认不侵蚀 6.9x 余量 | Accepted — §8 风险行 |
| M-R11-3 | A3 | 无 config 开关 enable_tickfile_commit_marker（不符合 enable_order_accel 惯例） | Accepted — §3.7 config flag |
| M-R11-4 | A3 | 容器化部署未覆盖（K8s 共享 PVC → flock 跨主机不互斥） | Accepted — §3.7 INV-CM-CONTAINER |
| M-R11-5 | A3 | metrics 仅内存硬崩溃丢；audit log 是唯一持久信号；须定义告警查询 + writer_perm_dead 路由 errors.log | Accepted — §3.7 监控方案 |

Minor：lockfile makedirs 顺序（recovery 在首次写前需 makedirs lockfile 目录）、rollback 前 systemctl stop、setup.sh fs 类型验证。
...
## Review Round 13（全新角度：跨输出一致性 / spec 可读性 / 对抗性单点突破）
...
| ID | 来源 | 问题 | 决议 |
| -- | ---- | ---- | ---- |
| C-R13-1 | A1 | **跨输出分歧**：crash 后 order/snapshot 文件存在（per-minute rename 原子）但 tickfile 被 recovery truncate → tickfile 永久缺该分钟。INV-CM-CHECKPOINT-RECONCILE 仅对账 snapshot↔tickfile（不含 order），且"注入 gap 补齐"未指定数据源（buffer 已空，数据在文件里）。 | Accepted — §3.8 扩展对账到三路 + 指定 regen 数据源 |
| C-R13-2 | A3 | **sidecar 被外部截断/误清 → live engine 无 _scan fallback → 全量重生 → 百万行重复**：replay 降级走 _scan 重建 skip-set，但 live engine 的 had_sidecar=False 降级路径未定义 _scan fallback（INV-CM-SKIPSET-LIVE 只说 committed_set 写入，committed_set=None 时写什么？空集？→ 全量重生）。 | Accepted — §3.8 INV-CM-SKIPSET-LIVE-FALLBACK + INV-CM-SIDECAR-TAMPER-DETECT |

#### Major
| ID | 来源 | 问题 | 决议 |
| -- | ---- | ---- | ---- |
| M-R13-1 | A1 | tickfile_only（tickfile 有、order/snapshot 缺）被当"理论上不应出现"但实际可发生（order-current-minute gate + async writer） | Accepted — §3.8 提升为可操作恢复 |
| M-R13-2 | A1 | output_minutes 语义歧义（snapshot 视图但 spec 记录为与 tickfile 顺序执行） | Accepted — §3.8 澄清 |
| M-R13-3 | A1 | 对账时序在 engine 启动中未锁定 | Accepted — §3.8 INV-CM-RECONCILE-ORDER |
| Mj-R13-1 | A2 | ~55 条 INV 无主索引表，无法逐条 verify | Accepted — §3.8 INV 总表 |
| Mj-R13-2 | A2 | §3.4-3.7 跨 Round 增补版本追踪困难 | Accepted — §3 版本地图 |
| Mj-R13-3 | A2 | _recover 返回值调用方接缝分散 5 条 INV | Accepted — §3.8 调用点接缝表 |
| Mj-R13-4(A2) | A2 | §7 ~31 条 in-file 测试（**第 4 次发现**，R9/R10/R12/R13）—— 已 accepted deferred plan Task 0 | 已接受（recurring，不重开） |

Minor：§4/§5/§6/§9 措辞残留 in-file（Mj-R13-2 配套）、lockfile 路径歧义、Mn-R13-* 等。
...
## Review Round 18（Round 17 修复后最终复审）

### 审核时间
* 2026-06-18 07:00:00

### Round 17 复核
全部 5 Critical（pandas 依赖 / 部署 runbook / .truncated 回滚 / 监控 3 方案 / 故障演练）+ 9 Major 在 §3.10 落实，涉及源码的项（config.py/flusher.py/replay.py/engine.py 行号、glob 谓词、循环导入、三 lazy seqno 入口）经代码级核实准确。0 Critical / 0 Major / 2 Minor（plan Task 0）。
...
| 7 | R13-14 | 跨输出/篡改/可读性 | 2C+6M | 全修 |
| 8 | R15-16 | cascade/plan/极端 | 3C+9M | 全修 |
| 9 | R17-18 | 耦合/测试/部署 runbook | 5C+9M | 全修 |

**累计**：28 Critical + 76 Major + ~40 Minor（全 Deferred 到 plan Task 0）。
...
## Review Round 19（全新角度：round-up 集成 / Phase 18 BG writer 交互 / fd 泄漏）
...
| ID | 来源 | 问题 | 决议 |
| -- | ---- | ---- | ---- |
| C-R19-1 | A1 | **Phase 22 round-up 边界混存**：旧 round-down tickfile + 新 round-up tickfile 同 output_dir → minute_key 语义不一致 → replay 重生/重复。spec §6 只考虑有/无 sidecar 混存，未考虑 round-up 语义变更混存。 | Accepted — §3.11 INV-CM-ROUNDUP-BOUNDARY + replay-to-fresh SHOULD + audit log legacy_updatetime |
| C-R19-2 | A1 | **engine.py:1458 health-check 实际不调 recovery**：spec INV-CM-ORDER-RESTART 要求 health-check restart 前调 recovery，但 engine.py 现状只有 `drain+new thread`，无 recovery → writer 死前 partial 永久残留。 | Accepted — §3.11 reconcile §3.4/§3.5：health-check 调 full recovery（选项 A） |

#### Major
| ID | 来源 | 问题 | 决议 |
| -- | ---- | ---- | ---- |
| M-R19-1 | A1 | `_tickfile_writer_pause` 无旧日 recovery 栅栏（C-R15-2 未在 engine.py 落地） | Accepted — §3.11 明确 pause 内 drain 后调 recovery |
| M-R19-2 | A1 | flock fd 须 `with _get_write_lock: with open(lockfile):` 嵌套模板 | Accepted — §3.11 INV-CM-FLOCK-WITH-NESTED + 代码骨架 |
| M-R19-3 | A1 | `_tickfile_writer_drain`（三路径共用）失败 partial 无 recovery 兜底 | Accepted — §3.11 drain 后可选 recovery + CRITICAL metric |

Minor：extract_minutes docstring round-up 注释 / prune RLock vs lockfile 协同 / health-check restart 上限措辞。

### Round 19 结论
**3. 需要修改后进行 Round 20 复审。**（2 Critical——C-R19-1 是 Phase 22 round-up 语义变更这一维度被 18 轮 review 完全漏掉；C-R19-2 是 spec INV 与 engine.py 实现的直接矛盾。两者都是"spec 说了但代码/集成层未落地"或"spec 漏了一个维度"。）

---

## Review Round 20（Round 19 修复后最终复审）

### 审核时间
* 2026-06-18 09:00:00

### Round 19 复核
全部 2 Critical + 3 Major 在 §3.11 落实。4/5 完整闭环；C-R19-2 的 §3.5 原文同步（Mj-R20-1）已修。

### 结论
**1. 可以进入 planning。** 0 Critical / 0 Major（Mj-R20-1 已修）。3 Minor deferred。
...
| 8 | R15-16 | cascade/plan/极端 | 3C+9M | 全修 |
| 9 | R17-18 | 耦合/测试/部署 runbook | 5C+9M | 全修 |
| 10 | R19-20 | round-up/BG writer/fd | 2C+3M | 全修 |

**累计**：28 Critical + 79 Major + ~43 Minor（全 Deferred 到 plan Task 0）。
...
## Review Round 21（全新角度：enable_tickfile=false 隔离 / 文件生命周期 / 全新读者可实现性）
...
| ID | 来源 | 问题 | 决议 |
| -- | ---- | ---- | ---- |
| C-R21-1 | A1 | INV-CM-ORDER-1 替换 flusher.py:98 时未显式 gate `enable_tickfile` → plan 作者可能把 recovery 移出 gate → enable_tickfile=false 时误跑 recovery 刷 CRITICAL + audit log 误报 | Accepted — §3.12 INV-CM-RECOVERY-GATE |
| C-R21-2 | A1 | `.truncated.*` 备份 + audit log 无 retention/rotation → 长期累积 + disk-full cascade 正反馈 | Accepted — §3.12 INV-CM-RETENTION + INV-CM-AUDIT-ROTATION |

#### Major
| ID | 来源 | 问题 | 决议 |
| -- | ---- | ---- | ---- |
| M-R21-1 | A1 | kill-switch 非 hot-read 一致性未声明 | Accepted — §3.12 INV-CM-KILLSWITCH-CONSISTENCY |
| M-R21-2 | A1 | 全新读者：缺源码改动总表（按文件维度） | Accepted — §3.12 §3.12.1 源码改动总表（plan Task 0） |
| M-R21-3 | A1 | 全新读者：缺实施顺序（layer 化 TDD 路径） | Accepted — §3.12 §3.12.2 实施 layer |
| M-R21-4 | A1 | 全新读者：~55 条 INV 未区分 code/doc/runbook 类型 | Accepted — §3.12 INV 总表加"实现类型"列（plan Task 0） |

Minor：enable_tickfile_commit_marker=True 但 enable_tickfile=False 的语义澄清、运维 du -sh 困惑文档化、write_tickfile_rows 传递性 gate 文档化。

### Round 21 结论
**3. 需要修改后进行 Round 22 复审。**（2 Critical + 4 Major。C-R21-1 是 enable_tickfile=false 时 recovery 误跑的安全隔离 gap；C-R21-2 是 .truncated.* + audit log 的无限累积 + disk-full cascade 正反馈；M-R21-2/3/4 是全新读者从 spec 写 plan 的高认知负荷——~55 条 INV 需按文件+类型+layer 重组。）

---

## Review Round 22（Round 21 修复后最终复审）

### 审核时间
* 2026-06-18 11:00:00

### Round 21 复核
全部 2 Critical（C-R21-1 enable_tickfile=false 隔离 / C-R21-2 文件 retention+rotation）+ 4 Major 在 §3.12 落实。0 Critical / 0 Major / 2 Minor（plan Task 0 执行提示）。
...
| 9 | R17-18 | 耦合/测试/部署 runbook | 5C+9M | 全修 |
| 10 | R19-20 | round-up/BG writer/fd | 2C+3M | 全修 |
| 11 | R21-22 | enable_false/生命周期/plan | 2C+4M | 全修 |

**累计**：28 Critical + 83 Major + ~44 Minor（全 Deferred 到 plan Task 0）。
...
## Review Round 23（全新视角终审：逻辑矛盾 / 简单失效场景 / 返回值一致性 / 遗漏路径 / 性能量化）
...
| ID | 问题 | 决议 |
| -- | ---- | ---- |
| C-R23-1 | REGEN-GUARD 分支 1b/2b 的 truncate 须在 flock 临界区内（非仅 _get_write_lock），否则跨进程交错损坏 | Accepted — §3.13 INV-CM-FLOCK-COVERS-REGEN-TRUNCATE |
| C-R23-2 | cross-day force-gen 失败的分钟数据丢失（_step1 except 仅 logger.exception 不 re-insert/retry，clear 后 pending 已 pop recovery 不重生成） | Accepted — §3.13 INV-CM-CROSSDAY-FORCEGEN-RETRY |

#### Major
| ID | 问题 | 决议 |
| -- | ---- | ---- |
| M-R23-1 | cross-day recovery 返回的旧日 committed_set 不应写 live skip-set | Accepted — §3.13 INV-CM-CROSSDAY-COMMITTED-DISCARD |
| M-R23-2 | 降级路径单次扫描合并 _extract_minutes + recover_tickfile_seqno | Accepted — §3.13 note |
| M-R23-3 | skip_fsync=True 下 INV-CM-MONO 落盘顺序保证失效 | Accepted — §3.13 INV-CM-SKIP-FSYNC-MONO-WEAKENED |

Minor：混存路径统一、FLOCK-LIFETIME 措辞"sidecar append"修正、HDD fsync 延迟阈值、enable_tickfile=False 时 scan fallback 跳过。
