...
> **Date**: 2026-06-17
> **Status**: ✅ 26 轮 review 完成（13 组 × 2 轮），可进入 planning。
...
## 12. Implementation Status (2026-06-22)

✅ **Implemented** on branch `feat/tickfile-commit-marker-recovery` (22 commits from base `cf63902`), following plan `docs/superpowers/plans/2026-06-18-tickfile-commit-marker-truncate-recovery.md`.

The sidecar design (§3.1/§3.2) shipped as specified. Additions/refinements discovered during implementation review (all documented in the plan's Post-Implementation Log):
- **Cross-day OLD-date recovery** made explicit in `_step1_cross_day_check` (the pause-path `_run_tickfile_recovery` resolved `jst_now` = the NEW date; added a direct old-date `_recover_tickfile_to_last_commit` call before the state clear, discarding the returned set). INV-CM-CROSSDAY-FLUSH-BARRIER fully met.
- **REGEN-GUARD extended for the empty-sidecar crash window (2A)**: `_classify_append_precondition` now detects an orphaned retry (sidecar empty/missing + tickfile non-empty + last row's minute == current) and truncates the orphaned current-minute block before rewriting — closing the INV-CM-ORDERED-TWO-FILE window where the tickfile is fsync'd but the sidecar write failed. Legacy rows (last row's minute != current) are preserved.
- **Kill-switch** (`enable_tickfile_commit_marker`) now threads through the live write path (`_try_generate_tickfile`) too, not just recovery/replay.
- **fstat** used for all tickfile-size reads that anchor truncation (INV-CM-OFFSET-FSTAT consistency across classify/strip/recovery).
- **Two real E2E tests** (`tests/e2e_tickfile_restart_recovery.py`): live+replay and live+live restart recovery, driving the real dataSimulator + real Engine + real ReplayEngine.

Verified correct: generated tickfile + E2E slice + pipeline coherence (8/8 + 6/6 + 4/4 checks, 2026-06-22). Production recovery topology: live self-heals TODAY's tickfile on restart (eager `__init__` recovery); `replay --date=<past>` backfills historical days; live(today) + replay(past) target different per-day tickfile files (no conflict); `fcntl.flock` per-date lockfile hard-aborts any same-date second writer (INV-CM-SINGLEPROC).
