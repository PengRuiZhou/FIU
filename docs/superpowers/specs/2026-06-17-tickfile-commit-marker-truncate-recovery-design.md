...
> **Date**: 2026-06-17
> **Status**: ✅ 9 轮 review（in-file R1-6 + sidecar R7-8 + 集成 R9），待 Round 10 复审。
...
>
> **⚠ Round 9 [DEPRECATED] 标注**：下方 §7 正文（Marker 写入 / Recovery 正确性 / Tail-check 协同 / 混存 / Live restart 等 ~15 条）仍保留**原始 in-file marker 断言**（如"末行是 `#COMMIT`"、"无 `#COMMIT` 行"）。这些条目**全部 [DEPRECATED]——plan Task 0 必须逐条重写为 sidecar 等价**（断言对象从"tickfile 末行 marker"改为"sidecar `.commit` 行数/末行/offset"）。在重写前，**实现者不得照搬这些断言**——照写会与 §3 sidecar 设计直接冲突（tickfile 无 marker 行，所有 `#COMMIT` 断言必失败）。以 §7 顶部 sidecar 专项测试 + §3.6 Round 9 集成增补中的测试名为准。
