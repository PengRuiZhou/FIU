...
首次需先装 Rust 工具链（`rust-toolchain.toml` 锁 1.84，cargo 首次自动拉取该 channel）：

```bash
# 仅首次：安装 Rust 工具链（minimal profile）
curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y --profile minimal
source "$HOME/.cargo/env"
# Linux 还需 C 链接器：RHEL/CentOS `sudo yum install -y gcc`，Debian/Ubuntu `sudo apt install -y gcc`
```
